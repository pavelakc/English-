...
async def gen_and_save_example(word_id, word_en, word_ru):
    try:
        await asyncio.sleep(1)
        prompt = ('Write one simple English sentence using "' + word_en + '" '
            '(meaning: ' + word_ru + '). '
            'Reply ONLY JSON: {"example":"sentence","translation":"Russian translation"}')
        text = await call_claude(prompt, 200)
        if not text:
            return
        ...
        if s >= 0 and e > s:
            ...
            if d.get("example"):
                await sb_patch("/rest/v1/words?id=eq." + word_id,
                    {"example_en": d["example"], "example_ru": d.get("translation","")})
                print("Example saved: " + word_en)
    except Exception as ex:
        print("Example error: " + str(ex))
...
async def save_word(user_id, word_en, word_ru):
    ...
    if result:
        ...
        asyncio.create_task(gen_and_save_example(wd["id"], word_en, word_ru))
        return wd
    return None
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not settings:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🇺🇦 Украинский", callback_data="native_uk")],
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not word_ru:
        word_ru = await auto_translate(word_en)
        if not word_ru:
            await msg.edit_text("❓ Не могу перевести. Напиши:\n`" + word_en + " - перевод`", parse_mode="Markdown")
            return
    result = await save_word(user_id, word_en, word_ru)
    if result:
        keyboard = [[InlineKeyboardButton("📚 Учить слова", web_app={"url": MINI_APP_URL})]]

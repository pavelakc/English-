...
async def auto_translate(word):
    if not ANTHROPIC_KEY:
        return ""
    prompt = 'Translate English word or phrase "' + word + '" to Russian. Reply ONLY JSON no markdown: {"ru":"перевод на русском","transcription":"[транскрипция]"}'
    text = await call_claude(prompt, 100)
    if not text:
        return ""
    try:
        clean = re.sub(r"```json|```", "", text).strip()
        d = json.loads(clean)
        return d.get("ru", ""), d.get("transcription", "")
    except:
        match = re.search(r'"ru"\s*:\s*"([^"]+)"', text)
        if match:
...
async def cmd_teacher(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if existing and len(existing) > 0:
        ...
        await update.message.reply_text(
            ...
            "🔑 Твой код: `" + user_id + "`\n\n"
            "Ученик вводит этот код в приложении во вкладке Итоги.",
            parse_mode="Markdown"
        )
        return
    ...
    await update.message.reply_text(
        ...
        "Ученик открывает Mini App -> Итоги -> вводит твой код\n\n"
        "*Отправить слова ученику:*\n"
        "Нажми кнопку 📤 Отправить слова",
        parse_mode="Markdown"
    )
...
async def cmd_quiz(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(w["word_ru"], callback_data="quiz_" + w["id"])] for w in options]
    await update.message.reply_text(
        "🧠 *Квиз!*\n\nКак переводится:\n*" + correct["word_en"] + "*",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(keyboard)
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(word_list) >= 2:
        ...
        for word_en, word_ru in word_list:
            ...
            if not word_ru:
                result_tr = await auto_translate(word_en)
                word_ru = result_tr[0] if isinstance(result_tr, tuple) else result_tr
            if not word_ru:
                skipped.append(word_en + " (?)"); continue
    ...
    if not word_ru:
        ...
        result_tr = await auto_translate(word_en)
        if isinstance(result_tr, tuple):
            word_ru, transcription = result_tr
        else:
            word_ru = result_tr
        if not word_ru:

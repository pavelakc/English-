...
async def auto_translate(word):
    if not ANTHROPIC_KEY:
        return ""
    prompt = 'Translate English word or phrase "' + word + '" to Russian. Reply ONLY JSON no markdown: {"ru":"перевод на русском"}'
    text = await call_claude(prompt, 80)
    if not text:
        return ""
    try:
        clean = re.sub(r"```json|```", "", text).strip()
        return json.loads(clean).get("ru", "")
    except:
        # Try to extract Russian text directly
